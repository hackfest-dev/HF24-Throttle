...
- Must always include a form for accept user input
- Must always include a date picker to ask when the house was constructed.
...
- To aid in fast user input when there are finite choices use the enum property to provide a list of options for each field, or if the answer can be parsed as a number then use number type.
  For example, instead of room size being one string input, it can be split into three number inputs: length and width and height. Here considering this always provide form never give simple text`,
                ...
                uiSchema: {
                    ...
                    type: 'object',
                    description: `Object of UI schema for customizing the form\'s presentation. Customize the layout and presentation of your form fields here, including widget types and help texts, always give a question with date picker to ask when the house was constructed. This schema controls the visual aspects of the form, enhancing user interaction and experience.
...
export async function POST(req: Request) {
    ...
    const openai = new OpenAI({
        apiKey: apiKey || process.env.OPENAI_API_KEY || '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',
    });

    const response = await openai.chat.completions.create({
        // model: 'gpt-4-0125-preview-0125',
...
Available tools:
- Interactive Map: Essential for insurance risk analysis, property risk analysis, event locations, buying home, bungalows  and potentially home automation control.
- 3D Rendering Engine: For interior design, home automation visualization, and potentially for event space planning.
- Customizable Forms/Input Components: To present to a user to ask them follow up questions that clarify their intent. Always generate a form.

Instructions: 
- If you need further context from the user to understand their intention sufficient enough to generate a good UI, respond with 5-7 relevant follow-up questions that you don't have the answer to already or statements to clarify the user's intention. Focus on understanding the specific requirements, preferences, or constraints related to their request. Don't ask unnecessary questions, never ask for name, email or phone number.
-  Always use 'create_simple_form' function, no matter how detailed the user's prompt is, ask only necessary info, always include a date picker to ask when the house was constructed, drop down menus, and value in INR, and include more necessary questions.
- Don't ask questions to which you have the answer already.`
